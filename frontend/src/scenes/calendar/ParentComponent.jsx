...
import ArrowDropDown from '@mui/icons-material/ArrowDownward'; // Importing a different icon
...
const ParentComponent = () => {
  ...
  const fetchEnrolledClasses = async () => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/enrolledClasses/${user.userID}`,
        {
          method: "GET",
          headers: {
  ...
  const addClass = async (newClass) => {
    ...
    if (newClass.lectures[0].discussions) {
      isClassAndLectureAndDiscussionAlreadyAdded = myClasses.some(
        (classInPlan) =>
          classInPlan.classId === newClass.id &&
          classInPlan.lectures.some(
            (lecture) =>
              lecture.num === newClass.lectures[0].num &&
              lecture.discussions.some(
                (discussion) =>
                  discussion.alpha === newClass.lectures[0].discussions.alpha
              )
          )
      );
    }
      ...
      (classInPlan) =>
        classInPlan.classId === newClass.id &&
        classInPlan.lectures.some(
          (lecture) => lecture.num === newClass.lectures[0].num
        )
    );
    }
    if (!isClassAndLectureAndDiscussionAlreadyAdded) {
      ...
      console.log(
        "This lecture (and discussion) has already been added to the plan."
      );
    }
  };
  ...
  const removeClass = async (classes) => {
    try {
      const response = await fetch(
        `http://localhost:4000/api/enrolledClasses/${classes._id}`,
        {
      ...
      setMyClasses((prevClasses) =>
        prevClasses.filter((c) => c.hash_id !== classes.hash_id)
      );
      //test
      console.log("Contents of myClasses:", myClasses);
    } catch (error) {
    ...
    return (
      ...
      >
        <CardContent sx={{ flexGrow: 1}}>
          ...
          <Typography variant="body1" component="div">
            Time: {myClass.lectures[0].time}, {myClass.lectures[0].days}
          </Typography>
          <Typography variant="body1" component="div">
          ...
          <Typography variant="body1" component="div">
            Final Exam: {myClass.lectures[0].final_date} at{" "}
            {myClass.lectures[0].final_time},{" "}
            {myClass.lectures[0].final_location}
          </Typography>
          
          
          {myClass.lectures[0]?.discussions?.length === 1 && (
            <>
              <Accordion variant="outlined" sx={{ 
                ...
                '&.MuiPaper-root': { boxShadow: 'none' }, // Remove shadow if present
                '&::before': { // Remove the default MUI Accordion's before element which adds a border-like effect
                    display: 'none',
                },
              ...
              />
              <Typography variant="body1" component="div">
                Discussion: {myClass.lectures[0].discussions[0]?.alpha}
              </Typography>
              <Typography variant="body1" component="div">
                Time: {myClass.lectures[0].discussions[0]?.time},{" "}
                {myClass.lectures[0].discussions[0]?.days}
              </Typography>
              <Typography variant="body1" component="div">
                Instructor:{" "}
                {myClass.lectures[0].discussions[0]?.instructors?.join(" ")}
              </Typography>
              <Typography variant="body1" component="div">
                Location: {myClass.lectures[0].discussions[0]?.location}
              </Typography>
              <Typography variant="body1" component="div">
                Status: {myClass.lectures[0].discussions[0]?.status}
              </Typography>
              {myClass.lectures[0].discussions[0]?.status?.toLowerCase() !==
                "closed" && (
                <Typography variant="body1" component="div">
                  Capacity: {myClass.lectures[0].discussions[0]?.capacity}
                </Typography>
              )}
              </AccordionDetails>
